...
export interface BambuA1Dimensions {
  ...
  Spool: { width: number; height: number; left: number; top: number };
}
...
export function getBambuA1Dimensions(config: any, bounds: { width: number; height: number }, haScaleFactor: number): BambuA1Dimensions {
  ...
  // Spool holder (on the right side)
  const SP_W = scale.val(config.spool.width);
  const SP_H = scale.val(config.spool.height);
  const SP_L = F_W - scale.val(config.spool.offsetRight) - SP_W;
  const SP_T = LID_H + scale.val(10);
  ...
  return {
    ...
    Spool: { width: SP_W, height: SP_H, left: SP_L, top: SP_T },
  };
}
