...
export interface CantileverDimensions {
  Bottom: { width: number; height: number; top: number };
  ZAxis: { width: number; height: number; left: number };
...
export function getCantileverDimensions(config: any, bounds: { width: number; height: number }, haScaleFactor: number): CantileverDimensions {
  ...
  const Z_H = scale.val(config.ZAxis.height) + B_H;
  const Z_L = scale.val(config.ZAxis.offsetLeft);

  const B_T = Math.ceil(Z_H - B_H);
  ...
  return {
    Bottom: { width: B_W, height: B_H, top: B_T },
    ZAxis: { width: Z_W, height: Z_H, left: Z_L },
    Scalable: { height: Z_H, width: B_W },
    BuildPlate: { width: P_W, left: P_L, top: P_T },
    XAxis: { width: X_W, height: X_H, left: X_L, top: X_T },
    Gantry: { width: G_W, height: G_H, left: G_L, top: G_T },
    Nozzle: { width: N_W, height: N_H, left: N_L, top: N_T },
    BuildArea: { width: A_W, height: A_H, left: A_L, top: A_T },
  };
}
